...
/**
 * Builds {@link LocalAccountFixtures} from the application's own beans. A regular
 * {@code @Component} under {@code src/test}, picked up by every {@code @SpringBootTest} of the
 * {@code io.opaa} package - not a {@code @TestConfiguration}, so importing it is never necessary
 * and no context's cache key changes.
 */
@Component
