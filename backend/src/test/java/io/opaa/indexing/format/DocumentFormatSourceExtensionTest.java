 ...
 */
class DocumentFormatSourceExtensionTest {
  ...
  void theDetectedExtensionWinsOverAMisleadingFileName() {
    ...
    DocumentFormatSource source =
        DocumentFormatSource.ofFile(Path.of("tabelle.csv"), "tabelle.csv", ".xlsx");
  ...
  void withoutADetectedExtensionTheFileNameSuffixIsUsedLowerCased() {
    DocumentFormatSource source =
        DocumentFormatSource.ofFile(Path.of("Nachricht.MSG"), "Nachricht.MSG");
  ...
  void aNameWithoutASuffixResolvesToNoExtension() {
    assertThat(DocumentFormatSource.ofFile(Path.of("tabelle"), "tabelle").effectiveExtension())
        .isNull();
    assertThat(DocumentFormatSource.ofExtractedText("text", "Schlagzeile").effectiveExtension())
        .isNull();
  }
}
