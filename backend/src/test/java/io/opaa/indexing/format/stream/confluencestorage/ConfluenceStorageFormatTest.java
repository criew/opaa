...
/**
 * The macro rule set and the structure preservation of {@link ConfluenceStorageFormat}, against a
 * representative storage-format page: headings, a table, lists, a code block, panels with titles,
 * an expand, a status lozenge, a task list, a link, an image - and the dynamic macros that must
 * leave no trace.
 */
class ConfluenceStorageFormatTest {
  ...
  void aPageOfNothingButDynamicMacrosHasNoExtractableText() {
    ...
    assertThat(pipeline.run(DocumentFormatSource.ofExtractedText("  ", "Leer")).outcome())
        .isEqualTo(DocumentFormatResult.Outcome.NO_CONTENT);
  }
  ...
  void emptyEditorLinesWithNonBreakingSpacesLeaveNoBlock() {
    ...
    assertThat(
            pipeline
                .run(DocumentFormatSource.ofExtractedText("<p>&nbsp;</p><p>\u00a0</p>", "Leer"))
                .outcome())
        .isEqualTo(DocumentFormatResult.Outcome.NO_EXTRACTABLE_TEXT);
  }
