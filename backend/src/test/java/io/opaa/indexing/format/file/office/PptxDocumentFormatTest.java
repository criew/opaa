 ...
 */
class PptxDocumentFormatTest {

  @TempDir Path tempDir;

  private final PptxDocumentFormat pipeline = new PptxDocumentFormat();
  ...
  void oneChunkPerSlideCarryingTitleAndSlideNumber() throws IOException {
    ...
    DocumentFormatResult result =
        pipeline.run(DocumentFormatSource.ofFile(file, "praesentation.pptx", ".pptx"));
  ...
  void aBlankSlideAlongsideOthersStillBecomesItsOwnChunk() throws IOException {
    ...
    DocumentFormatResult result =
        pipeline.run(DocumentFormatSource.ofFile(file, "mit-leerer-folie.pptx", ".pptx"));
  ...
  void aPresentationWhereNoSlideHasAnyTextIsRejectedAsNoExtractableText() throws IOException {
    ...
    DocumentFormatResult result =
        pipeline.run(DocumentFormatSource.ofFile(file, "nur-bilder.pptx", ".pptx"));
  ...
  void aTableIsReadRowByRowAndAddedAsBodyText() throws IOException {
    ...
    DocumentFormatResult result =
        pipeline.run(DocumentFormatSource.ofFile(file, "tabelle.pptx", ".pptx"));
  ...
  void aGroupShapeIsDescendedIntoRecursively() throws IOException {
    ...
    DocumentFormatResult result =
        pipeline.run(DocumentFormatSource.ofFile(file, "gruppe.pptx", ".pptx"));
  ...
  void theTitleShapeIsExcludedByIdentityNotByTextEquality() throws IOException {
    ...
    DocumentFormatResult result =
        pipeline.run(DocumentFormatSource.ofFile(file, "wiederholter-titel.pptx", ".pptx"));
  ...
  void aNotesTextBoxWithoutAPlaceholderTypeIsIncludedWithoutThrowing() throws IOException {
    ...
    DocumentFormatResult result =
        pipeline.run(DocumentFormatSource.ofFile(file, "notiz-textfeld.pptx", ".pptx"));
  ...
  void aPresentationWithoutAnySlideHasNoContent() throws IOException {
    ...
    DocumentFormatResult result =
        pipeline.run(DocumentFormatSource.ofFile(file, "keine-folien.pptx", ".pptx"));
  ...
  void aFileThatIsNotAValidPptxIsAParseFailure() throws IOException {
    ...
    DocumentFormatResult result =
        DocumentFormatRunner.run(
            pipeline, DocumentFormatSource.ofFile(file, "kaputt.pptx", ".pptx"));
  ...
  void aFilelessSourceIsAParseFailure() {
    ...
    // DocxDocumentFormat.
    DocumentFormatResult result =
        pipeline.run(DocumentFormatSource.ofExtractedText("irrelevanter Text", "quelle.pptx"));
  ...
  void readsCorePropertiesAndTheFirstSlideTitleAsDocumentProperties() throws IOException {
    ...
    DocumentFormatSource source = DocumentFormatSource.ofFile(file, "vortrag.pptx", ".pptx");

    io.opaa.indexing.format.DocumentProperties properties = pipeline.readProperties(source);
