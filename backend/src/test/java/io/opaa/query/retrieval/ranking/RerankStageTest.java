...
class RerankStageTest {
  ...
  private static QueryProperties properties(int rerankCandidateCount) {
    return new QueryProperties(TOP_K, 25, 1.0, 0.3, false, 3, 2, true, rerankCandidateCount, 20, 2);
  }
  ...
  private static RetrievalState stateWith(int chunkCount) {
    ...
    return RetrievalState.initial()
        .withCandidateLists(
            List.of(new CandidateList(RetrievalListLabel.FUSED_LIST_LABEL, documents)));
  }
  ...
  void aSwitchedOffStageLeavesSeveralListsAndTheirLabelsAlone() {
    RetrievalState state =
        RetrievalState.initial()
            .withCandidateLists(
                List.of(
                    new CandidateList(
                        RetrievalListLabel.VECTOR_SEARCH.format(1), List.of(chunk("v0"))),
                    new CandidateList(
                        RetrievalListLabel.FULL_TEXT_SEARCH.format(1), List.of(chunk("f0")))));
    ...
    assertThat(outcome.state().candidateLists())
        ...
        .containsExactly(
            RetrievalListLabel.VECTOR_SEARCH.format(1),
            RetrievalListLabel.FULL_TEXT_SEARCH.format(1));
