 ...
 * question, its own hand-written answer and its own expected documents.
 *
 ...
 * said, not only against what was asked. The hand-written answer makes the window deterministic and
 * the case reproducible without a generation step. It is one to three sentences in most classes and
 * a long, production-shaped answer in {@code answer_continuation} (see {@link
 * ConversationCaseCuration#LONG_ANSWER_RULE}).
 *
 * <p>A case is solved when <b>every</b> one of its turns is solved under the existing criterion
 * ({@link ExpectedStateAudit#isSolved}), or - for a turn without search - when nothing was
 * searched; the state fields describe the case, never a single turn. They stay flat: this dataset
 * is measured on the pipeline path only, so its one declared state is that path's state
 * (docs/features/retrieval-benchmark.md §5, Einpfad-Regel).
 *
...
public record ConversationCase(
   ...
   * One turn of a case.
   *
   * @param answer the hand-written answer that enters the conversation window as the assistant's
   *     turn - see the class Javadoc.
   ...
   * @param searchExpected {@code false} for a message without anything to search for (an
   *     instruction on the answer form, a statement about the asking person, thanks): such a turn
   *     carries no expected documents and is solved exactly when the run did not search. Absent
   *     means {@code true}.
   */
  @JsonIgnoreProperties(ignoreUnknown = true)
  public record Turn(
      ...
      @JsonProperty("search_expected") Boolean searchExpected) {

    /**
     * A turn that expects a search - the form of every turn outside {@code answer_continuation}.
     */
    public Turn(
        String query, String answer, List<String> expectedDocuments, String confusableDocument) {
      this(query, answer, expectedDocuments, confusableDocument, null);
    }

    /** Whether the run is expected to search for this turn at all. */
    public boolean expectsSearch() {
      return searchExpected == null || searchExpected;
    }
  }
