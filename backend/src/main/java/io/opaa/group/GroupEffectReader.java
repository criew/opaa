...
class GroupEffectReader {
  ...
  List<SuccessionFinding> grantsWithoutRecipient(UUID organizationId) {
    ...
    for (Group group : effects.groups()) {
      ...
      boolean reaches = effects.reaches(group);
      boolean effective =
  ...
  List<SuccessionFinding> groupsWithoutEffect(UUID organizationId) {
    ...
    for (Group group : effects.groups()) {
      ...
      if (!holdsSomething && !effects.reaches(group)) {
  ...
  private Effects effectsOf(UUID organizationId) {
    // With memberships, so the accounts of every group of the organization are decided in one
    // query instead of one per group (#682's rule, applied to the two group tabs).
    List<Group> all = groups.findByOrganizationIdWithMemberships(organizationId);
    if (all.isEmpty()) {
      return new Effects(List.of(), Map.of(), Set.of(), Set.of());
    }
    ...
    return new Effects(
        all,
        objectsByGroup,
        Set.copyOf(capabilities.findSubjectGroupIdsIn(groupIds)),
        activeMembers);
  }
  ...
  private record Effects(
      List<Group> groups,
      Map<UUID, Set<UUID>> objectsByGroup,
      Set<UUID> holdingCapabilities,
      Set<UUID> activeMemberIds) {

    /**
     * Whether the group reaches at least one usable account - the same definition {@code
     * GroupMembershipResolver#activeMemberIds} applies, decided here for the whole organization at
     * once.
     */
    boolean reaches(Group group) {
      return group.getMemberships().stream()
          .anyMatch(membership -> activeMemberIds.contains(membership.getUserId()));
    }
  }
}
