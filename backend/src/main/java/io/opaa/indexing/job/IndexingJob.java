...
public class IndexingJob {
   ...
   * to its target after the fact. Nullable because a target library can later be deleted ({@code ON
   * DELETE SET NULL}).
   */
   ...
   * io.opaa.indexing.job.IndexingJobService#startJob}, mirroring {@link #libraryId} but never left
   ...
   * io.opaa.indexing.job.IndexingJobService#startJob(java.util.UUID, java.util.UUID,
   * JobTriggerSource, io.opaa.api.types.IndexingRunMode)} was called with {@link
   * JobTriggerSource#SCHEDULED}. {@code KnowledgeLibraryService} uses this to compute {@code
   * LibraryResponse.lastScheduledRunsFailed} without conflating a manual retry with the scheduled
   * runs it retried after.
   */
  @Enumerated(EnumType.STRING)
