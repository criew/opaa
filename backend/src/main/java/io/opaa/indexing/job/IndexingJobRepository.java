...
public interface IndexingJobRepository extends JpaRepository<IndexingJob, UUID> {
  ...
  @Query(
      "select j.libraryId as libraryId, max(j.completedAt) as lastCompletedAt from IndexingJob j"
          + " where j.libraryId in :libraryIds and j.status = io.opaa.indexing.job.JobStatus.COMPLETED"
  ...
  @Query(
      "update IndexingJob j set j.status = io.opaa.indexing.job.JobStatus.FAILED, j.errorMessage ="
          + " :errorMessage, j.completedAt = :completedAt where j.status ="
          + " io.opaa.indexing.job.JobStatus.RUNNING and j.lastProgressAt < :cutoff")
  ...
  @Transactional
  @Query(
      "update IndexingJob j set j.status = io.opaa.indexing.job.JobStatus.COMPLETED,"
