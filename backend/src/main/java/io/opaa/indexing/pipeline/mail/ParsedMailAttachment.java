 ...
 * #tempFile()} once it has routed the attachment through {@link DocumentPipelineRegistry}.
 *
 * @param fileName the attachment's own file name, as carried by the message - never blank (a reader
 *     that cannot resolve one names it itself, so {@link SupportedDocumentFormats} always has
