 ...
 * metadata} for the schema fields and {@code attachment} for discovered attachments. Of {@code job}
 * it knows only the protocol sink it reports each document's outcome to; the run, its schedule and
 * its progress stay there. {@code maintenance} depends on this package, never the other way round.
 */
