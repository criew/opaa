 ...
 * to. {@code title} and {@code description} may be blank if the feed omits them. {@code
 * publishedAt} is empty both when {@code pubDate} is absent and when it could not be parsed - an
 * unreadable date must not invalidate the entry (ADR-0017, #466).
 */
