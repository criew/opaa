...
/**
 * Either a core field or a library field (#1071), reduced to what everything keyed on {@code
 * field_key} needs: the stored key, the German label for display and audit, and - for a library
 * field - the field id the value row must reference. The one place the two kinds of field meet, so
 * that setting, deleting, auditing and counting a value stay one code path.
 *
