...
/**
 * Routes an admitted document to the {@link DocumentFormat} responsible for its format
...
public class DocumentFormatRegistry {
  ...
  private static final Logger log = LoggerFactory.getLogger(DocumentFormatRegistry.class);

  private final Map<String, DocumentFormat> byFormat;
   ...
   */
  public DocumentFormatRegistry(List<DocumentFormat> pipelines, DocumentFormat fallback) {
    ...
    this.all =
        pipelines.contains(fallback)
            ...
            : java.util.stream.Stream.concat(
                ...
                .distinct()
                .toList();
    Map<String, DocumentFormat> claims = new HashMap<>();
    for (DocumentFormat pipeline : all) {
      if (pipeline == fallback) {
        continue;
      }
      for (String format : pipeline.handledFormats()) {
        DocumentFormat previous = claims.put(format, pipeline);
        if (previous != null) {
   ...
   * hand both to {@link DocumentFormatSource#ofFile(Path, String, String)}. Kept as a nested record
   * rather than a two-element return, so a future addition to what routing resolves does not ripple
   * through every call site's argument list.
   *
  ...
  public record Routed(
    ...
    public Routed(DocumentFormat pipeline, String detectedExtension) {
   ...
   */
  public DocumentFormat pipelineFor(Path file, String fileName) {
   ...
   */
  public DocumentFormat pipelineFor(String fileName, String detectedMediaType) {
  ...
  private Routed routedPipelineFor(String fileName, String detectedMediaType) {
    ...
    DocumentFormat pipeline = byFormat.getOrDefault(decision.detectedExtension(), fallback);
   ...
   */
  public DocumentFormat fallbackPipeline() {
    return fallback;
  }
   ...
   * is a file.
   */
  public java.util.Optional<DocumentFormat> pipelineById(String id) {
   ...
   *
   * @param routingExtension {@code null} or {@link ChunkFormatMetadata#NO_ROUTING_EXTENSION} for a
   *     chunk whose routing never resolved an extension - resolves to {@link #fallbackPipeline()}
  ...
  public Collection<DocumentFormat> pipelines() {
    return all;
  }

  /**
   * The union of every registered pipeline's {@link DocumentFormat#passthroughMetadataKeys()},
