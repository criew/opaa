 ...
 * via {@link DocumentFormatResult#discoveredAttachments()}, and recursion and the count/depth
 * limits live on the shared attachment path.
 */
public class MailDocumentFormat implements DocumentFormat {

  private static final Logger log = LoggerFactory.getLogger(MailDocumentFormat.class);
  ...
  /**
   * Honours {@link DocumentFormatSource#attachmentIndex()} by handing it to the reader: the
  ...
  public DocumentFormatResult run(DocumentFormatSource source) {
    if (source.file() == null) {
      ...
      // TabularDocumentFormat's identical guard.
  ...
  private DocumentFormatResult resultOf(ParsedMailMessage message, String fileName) {
    ...
    if (chunks.isEmpty()) {
      ...
      // discoveredAttachments still carries the attachment: DocumentFormatResult reserves
      ...
      return DocumentFormatResult.noExtractableText(discovered);
    }
    ...
    return DocumentFormatResult.chunked(chunks, discovered, contentByteSize)
        .withProperties(properties(message));
  }

  /**
   * A wanted index no attachment position can ever equal - {@link #readProperties} only needs the
   * Kopfdaten, and does not run through {@link io.opaa.indexing.format.DocumentFormatRunner}, so
   * any temp file it caused a reader to write would never be deleted by anyone.
   */
