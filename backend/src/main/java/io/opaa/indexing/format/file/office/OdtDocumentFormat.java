 ...
 * DocxDocumentFormat}, reading {@code content.xml} through the hardened SAX parser {@link
 ...
 */
public class OdtDocumentFormat extends FileDocumentFormat<OdtDocumentFormat.OdtContent> {
  ...
  /** Cutting stops at level 3, mirroring {@link DocxDocumentFormat#MAX_CUTTING_LEVEL}. */
  ...
  public OdtDocumentFormat(OdfProperties odfProperties) {
    this(odfProperties, OdfPackage::open);
  }

  OdtDocumentFormat(OdfProperties odfProperties, OdfPackage.Opener opener) {
  ...
  /**
   * One document's reading, all from one opening of the package: {@code content.xml}'s event
   * stream, {@code meta.xml}'s data and {@code styles.xml}'s header/footer text ("" when absent).
   */
  public record OdtContent(
  ...
  private List<HeadingSectionSplitter.Event> readEvents(OdfPackage odf, String fileName)
      throws IOException {
    ...
    if (!odf.parse("content.xml", odfProperties.maxContentXmlBytes(), handler)) {
      ...
      throw new IOException("No content.xml entry in ODT file " + fileName);
    }
   ...
   * title and dates. {@code styles.xml} is not read - the Bestandslauf needs no header/footer.
   */
  @Override
  ...
  @Override
  protected DocumentFormatResult chunks(DocumentFormatSource source, OdtContent content) {
    ...
    if (chunks.isEmpty()) {
      ...
      // same NO_EXTRACTABLE_TEXT outcome TikaFallbackFormat reported for either case before this
   ...
   */
  private String readHeaderFooterText(OdfPackage odf, String fileName) {
