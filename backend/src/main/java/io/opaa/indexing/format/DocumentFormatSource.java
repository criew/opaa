...
/**
 * What a {@link DocumentFormat} is handed: either a file on disk, or text that was already
 * extracted upstream and never had a file (a feed entry's main content as HTML, a Confluence page
 * body - see {@code DocumentIngestService#ingest}).
 *
 ...
 *     text-tolerant part of the routing rule (see {@link DocumentFormatRegistry})
 * @param detectedExtension the extension {@link DocumentFormatRegistry} actually routed on (see
 *     {@link DocumentFormatRegistry.Routed#detectedExtension()}), or {@code null} when routing did
 *     not resolve one (detection failed, or this source never went through the registry at all -
 *     the {@link #ofExtractedText} of a feed entry or a Confluence page). A pipeline handling more
 *     than one format (e.g. {@code TabularDocumentFormat}'s XLSX/CSV/ODS) dispatches on this, not
 *     on {@code fileName}'s own suffix - a document is admitted and routed by its <em>detected</em>
 *     content , and a pipeline that re-derived the format from the name alone would silently
 ...
 *     reports {@link DocumentFormatResult#discoveredAttachments()} must honour it: it numbers
...
public record DocumentFormatSource(
  ...
  public static DocumentFormatSource ofFile(Path file, String fileName) {
    return new DocumentFormatSource(fileName, file, null, null, null);
  }

  public static DocumentFormatSource ofFile(Path file, String fileName, String detectedExtension) {
    return new DocumentFormatSource(fileName, file, null, detectedExtension, null);
  }

  public static DocumentFormatSource ofExtractedText(String extractedText, String fileName) {
    return new DocumentFormatSource(fileName, null, extractedText, null, null);
  }
}
