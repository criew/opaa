 ...
 * chunk_full_text} row - the presence or absence of that row <em>is</em> the persisted progress.
 * Calling this method again after a crash, a restart or simply the next scheduled tick re-derives
 * exactly the remaining work from the database; nothing is lost, and nothing already done is redone
 * (a chunk that made it into {@code chunk_full_text} on a previous, interrupted call is excluded
 * from every later batch). This is also why {@link FullTextChunkStore#indexChunks}'s own {@code ON
 * CONFLICT (chunk_id) DO NOTHING} matters here: even if two calls raced on the same chunk (e.g.
 * this backfill and a concurrent re-index of the same chunk on the write path, an unlikely but
 * possible overlap since chunk ids are freshly generated per write - see {@code
 * FileProcessingService#storeChunks}), neither raises a primary-key violation.
 *
...
public class FullTextBackfillService {
  ...
  public FullTextBackfillService(JdbcTemplate jdbcTemplate, FullTextChunkStore fullTextChunkStore) {
    this.jdbcTemplate = jdbcTemplate;
    this.fullTextChunkStore = fullTextChunkStore;
  }

  /**
   * Indexes up to {@code batchSize} not-yet-full-text-indexed chunks and returns how many it
   * actually processed - {@code 0} means the backlog is empty (as of this call; new chunks written
   * after #1047 never add to it, since they are indexed at write time already). Chunks missing the
   * {@code document_id}/{@code library_id} metadata keys are excluded rather than failing the whole
   * batch - {@code FileProcessingService} has written both on every chunk since long before #1047,
   * so this only guards a hypothetical malformed row, not an expected case.
   */
  @Transactional
    ...
    List<org.springframework.ai.document.Document> pending =
        jdbcTemplate.query(
            "SELECT id, content, metadata->>'document_id' AS document_id, "
                ...
                + "FROM vector_store v "
                + "WHERE NOT EXISTS (SELECT 1 FROM chunk_full_text f WHERE f.chunk_id = v.id) "
