 ...
 * requires. {@code io.opaa.api.GlobalExceptionHandler} maps it to {@code 403} with {@link
 * #getMessage()} as the user-facing text and, when given, {@link #getCode()} as the stable,
 * machine-readable {@code code} of the response (e.g. {@code CAPABILITY_REQUIRED}) - for the cases
 * a client has to act on rather than display. Distinct from {@code
