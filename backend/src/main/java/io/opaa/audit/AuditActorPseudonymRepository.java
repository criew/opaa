...
public interface AuditActorPseudonymRepository extends JpaRepository<AuditActorPseudonym, UUID> {
   ...
   * keyed off a unique constraint on {@code user_id} - {@code ON CONFLICT ... DO NOTHING} lets two
   * concurrent first-audit-event calls for the same user race safely instead of one failing with a
   * unique-constraint violation. Used only by {@link AuditActorPseudonymService#pseudonymFor},
   * which always re-reads via {@link #findByUserId} afterwards to return whichever row won.
   */
  @Modifying
  @Query(
      value =
