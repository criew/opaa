 ...
 * title generation exactly when it takes effect for {@code AnswerGenerationService} - the next
 * request either way, never a restart. {@code io.opaa.llm.NoActiveChatModelException} is a {@link
 * RuntimeException}, so it is caught by {@link #generateTitleAsync}'s existing catch-all exactly
 * like any other title-generation failure.
 */
@Service
