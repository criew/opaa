 ...
 * knows no {@code IS NULL}, so the vector form expresses absence as {@code NOT IN} over the
 * complete set of values the key can carry - the Dokumentart vocabulary, or the three precisions -
 * which is true exactly for a chunk without the key.
 *
...
final class MetadataFilterExpressions {
  ...
  static Filter.Expression vectorExpression(
    ...
    if (filter.filtersDocumentType()) {
      ...
      if (!unselected.isEmpty()) {
        combined = b.nin(CoreMetadataChunkKeys.DOCUMENT_TYPE, unselected);
      }
    }
  ...
  static Filter.Expression subordinateTo(
    ...
    return new Filter.Expression(Filter.ExpressionType.AND, libraryFilter, metadataExpression);
  }
   ...
   * order the fragment consumes them. Empty for a filter that constrains nothing.
   */
  static String sqlPredicate(
      MetadataFilter filter, String metadataColumn, List<Object> parameters) {
    ...
    if (filter.filtersDocumentType()) {
      ...
      sql.append(" AND (")
          ...
          .append(typeKey)
          .append(" = ANY(?))");
      parameters.add(filter.documentTypes().toArray(String[]::new));
    }
    if (filter.filtersDocumentDate()) {
      ...
      sql.append(" AND (").append(dateKey).append(" IS NULL");
      for (DatePrecision precision : DatePrecision.values()) {
  ...
  /** The two vocabulary codes lists in insertion order, for the protocol note. */
  static String describeTypes(MetadataFilter filter) {
    return String.join(", ", new ArrayList<>(filter.documentTypes()).stream().sorted().toList());
  }
}
