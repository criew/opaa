 ...
 * standard validators (expiry, issuer) plus {@link AuthorizedPartyValidator}. The JWK set itself is
 * read through a client that never follows redirects, so neither address can lead anywhere the
 * policy did not see. With an override no discovery happens at all, which is what makes the Compose
 * split ({@code keycloak:8180} for the backend, {@code localhost:8180} for the browser) work.
 */
