 ...
 * {@code GlobalExceptionHandler}'s existing {@code ServiceUnavailableException} handler turns it
 * into the same German, user-facing error body every other domain error in this codebase already
 * produces, and {@code ChatTitleGenerationService#generateTitleAsync}'s existing catch-all already
 * swallows it exactly like any other title-generation failure.
 */
