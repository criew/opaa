...
| upload-store | 8095 (API), 8096 (Konsole unter `/rustfs/console/`) | 9000, 9001 | Objektspeicher der Originalablage (nur `upload-s3`-Profil; an `127.0.0.1` gebunden, siehe [Originalablage](#originalablage)) |
...
| `OPAA_UPLOAD_S3_KEY_PREFIX` | — (leer) | nicht gesetzt | Präfix vor jedem Objektschlüssel (`opaa.upload.s3.key-prefix`), z. B. `uploads/` — mit abschließendem `/`, wenn ein Ordner gemeint ist. Trennt mehrere Installationen in einem Bucket |
| `OPAA_UPLOAD_S3_PATH_STYLE` | `true` | nicht gesetzt | Adressstil (`opaa.upload.s3.path-style`): `true` = `endpoint/bucket/schlüssel` (MinIO, Ceph, RustFS), `false` = `bucket.endpoint/schlüssel` (AWS, Hetzner) |
...
| `OPAA_QUERY_METADATA_FILTER_LIBRARY_FIELD_THRESHOLD` | `0.75` | `0.75` | Dieselbe Eintrittsbedingung für jedes Bibliotheksfeld — Bezugsmenge ist die **eigene Bibliothek des Feldes**, nicht der ganze Suchbereich. Dieselbe Zahl wie Datum/Stand: Ein Bibliotheksfeld wird von Hand gepflegt, nie extrahiert, deshalb gilt die höhere Dokumentart-Schranke hier nicht (siehe [Metadaten](metadaten.md)) |
...
| **Suchen und Abrufen ohne Antwort** | | | |
| `OPAA_SEARCH_DEFAULT_MAX_HITS` | `10` | `10` | Treffer, die `POST /api/v1/search` liefert, wenn die Anfrage keine eigene Zahl nennt. Mehr als die Endauswahl (`OPAA_QUERY_TOP_K`) gibt es nie — der Suchweg schneidet aus ihr aus, er sucht nicht weiter |
| `OPAA_SEARCH_MAX_HITS` | `50` | `50` | Serverseitige Obergrenze für die von der Anfrage gewünschte Trefferzahl. Eine größere Anfrage wird darauf gekürzt, nicht abgelehnt: Ein fremdes Werkzeug, das eine runde Zahl nennt, soll eine Antwort bekommen, keinen Fehler |
...
| `OPAA_MCP_DEFAULT_MAX_HITS` | `5` | `5` | Dokumente, die eine Suche über den MCP-Server ohne eigenes `maxHits` liefert. Niedriger als `OPAA_SEARCH_DEFAULT_MAX_HITS`, weil ein fremdes Modell jedes gelieferte Zeichen aus seinem Kontextfenster bezahlt und den gewählten Abschnitt ohnehin nachlädt. Muss positiv sein — der Start bricht sonst ab |
| `OPAA_MCP_EXCERPT_CHARACTERS` | `500` | `500` | Länge des Auszugs um die Fundstelle je Treffer des MCP-Suchwerkzeugs. Betrifft nur diesen Kanal; `POST /api/v1/search` antwortet unverändert mit `OPAA_SEARCH_EXCERPT_MAX_CHARACTERS`. Mindestens `100` — darunter bricht der Start ab |
| **Fremdzugänge** | | | |
...
| `OPAA_INDEXING_TARGET_VALIDATION_ALLOWLIST` | — (leer) | nicht gesetzt (auskommentiert; Beispielwert `intranet.example.org`) | Hostnamen (kommagetrennt, exakter Vergleich ohne Groß-/Kleinschreibung), die von der Zielprüfung oben ausgenommen sind, auch während sie aktiv ist — erlaubt konkrete interne Quellen zu benennen, ohne die Prüfung für jedes andere Ziel abzuschalten. Für einen `S3`-Objektspeicher im Haus (MinIO, Ceph) gehört der Endpoint-Host hinein, unter Virtual-Host-Adressierung zusätzlich jeder Bucket-Host `<bucket>.<endpoint-host>` ([Konnektor S3](konnektor-s3.md), Abschnitt 4.1); der Demo-Stack nennt so seinen `objectstore`-Service. Ein selbst betriebenes Confluence Data Center steht typischerweise in einem privaten Adressbereich und braucht diesen Eintrag; die Fehlermeldung der Confluence-Zugriffsschicht nennt ihn ([Konnektor Confluence](konnektor-confluence.md)). |
...
| **Berechtigungen** | | | |
| `OPAA_MINIMUM_GROUP_SIZE` | `5` | `5` | Mindestgruppengröße für einen Rechteprofil-Lauf der Suchdiagnose **mit Space-Kontext** (ADR-0036): So viele aktive Konten der Gruppe müssen den Space auf irgendeinem Weg erreichen, sonst antwortet der Endpunkt mit `403` und verweist auf den Personenkontext mit Vollmacht. **`5` ist zugleich Voreinstellung und erzwungene Untergrenze** — der Wert ist nur nach oben änderbar; ein Startversuch mit einem kleineren Wert bricht mit einer Meldung ab. Ein Haus mit kleinen Einheiten hebt ihn an; abschalten kann ihn niemand. Eine Änderung wirkt erst mit dem Neustart und erzeugt **keinen** Protokolleintrag — sie ist organisatorisch wie ein Governance-Ereignis zu behandeln und nur über die Konfiguration der Installation nachweisbar. |
| `OPAA_SUCCESSION_AGING_THRESHOLD_MONTHS` | `12` | `12` | Ab welchem Alter ein Eintrag der Betriebsliste „Nachfolge offen" hervorgehoben wird — und wie lange ein Sichtungsvermerk („geprüft am …, weiterhin offen, Grund") die Hervorhebung wieder aufhebt (ADR-0036, Entscheidung 6). Die Schwelle **hebt hervor und löst nichts aus**: keine Frist, keine Eskalation, keine E-Mail. |
| `OPAA_SUCCESSION_DETECTION_CRON` | `0 5 * * * *` | `0 5 * * * *` | Wann der Feststellungslauf nach offenen Nachfolgen, Freigaben ohne Empfänger und Gruppen ohne Wirkung sieht (Spring-Cron, sechs Felder). Er schreibt **nur** Erstfeststellung und Ende eines Vorgangs — der Zustand selbst ist abgeleitet und gilt auch dann, wenn der Lauf nie lief; ohne ihn fehlte den Einträgen nur das Alter. |
| **pgvector** | | | |
...
| `OPAA_RATE_LIMIT_CHAT_SEARCH_MAX_REQUESTS` | `60` | nicht gesetzt (Anwendungs-Default gilt) | Max. Chatsuchen (`POST /api/v1/spaces/{spaceId}/chats/search`) pro IP pro Fenster, über alle Spaces zusammen — eigene Grenze, weil die Suche beim Tippen läuft (siehe [Suche](suche.md#101-ratenbegrenzung)) |
| `OPAA_RATE_LIMIT_CHAT_SEARCH_WINDOW_SECONDS` | `60` | nicht gesetzt (Anwendungs-Default gilt) | Chatsuche-Rate-Limit-Fenster in Sekunden |
| `OPAA_RATE_LIMIT_CHAT_SEARCH_GLOBAL_MAX_REQUESTS` | `1200` | nicht gesetzt (Anwendungs-Default gilt) | Max. Chatsuchen über alle IPs pro Fenster |
...
| `OPAA_RATE_LIMIT_LOCAL_AUTH_HANDOVER_MAX_REQUESTS` | `60` | nicht gesetzt (Anwendungs-Default gilt) | Max. Aufrufe der Übergabe eines lokalen Kontos an eine Anbieteridentität pro Client-Adresse pro Fenster. Vorschau und Einlösung teilen sich diesen Topf, je Person fallen also zwei Aufrufe an. Bewusst weiter als die übrigen Link-Flüsse: Die Übergabe ist der Weg einer ganzen Umstellung, und eine Behörde sitzt hinter einer Adresse — für eine Sammelumstellung vorübergehend anheben |
| `OPAA_RATE_LIMIT_LOCAL_AUTH_HANDOVER_WINDOW_SECONDS` | `900` | nicht gesetzt (Anwendungs-Default gilt) | Fenster der Übergabegrenze in Sekunden |
...
| `OPAA_DEMO_OBJECTSTORE_PORT` | — (kein Spring-Property; nur `docker-compose.yml`, dort Compose-Default `8093`) | wirkt nur aus Prozessumgebung/`.env`, **nicht** aus `.env.docker` (siehe Hinweis oben) — nicht in `.env.docker.example` gesetzt; ohne Shell-Export gilt der Compose-Default `8093` | Host-Port der S3-API des Demo-Objektspeichers `objectstore` (nur `demo`-Compose-Profil) — zum Prüfen mit `aws s3` vom Host; das Backend erreicht `objectstore` über das Compose-Netzwerk unter dem Servicenamen, der dafür in `OPAA_INDEXING_TARGET_VALIDATION_ALLOWLIST` stehen muss. Der Dienst hält die Quellen beider `S3`-Bibliotheken **und** die Originalablage der Demo; für Letztere passiert die eigene Zieladressprüfung der Ablage ihre konfigurierte Adresse ohne Allowlist-Eintrag |
| `OPAA_DEMO_OBJECTSTORE_CONSOLE_PORT` | — (kein Spring-Property; nur `docker-compose.yml`, dort Compose-Default `8094`) | wirkt nur aus Prozessumgebung/`.env`, **nicht** aus `.env.docker` (siehe Hinweis oben) — nicht in `.env.docker.example` gesetzt; ohne Shell-Export gilt der Compose-Default `8094` | Host-Port der Konsole des Demo-Objektspeichers (nur `demo`-Compose-Profil) — zum Ansehen seiner drei Buckets im Browser: `rheinfurt-archiv` und `formattest` als Quellen der beiden `S3`-Bibliotheken sowie `opaa-uploads` als Originalablage der Demo. Anmeldung mit den Demo-Zugangsdaten des Compose-Stacks. Der Speicher serviert die Konsole unter dem Pfad `/rustfs/console/`, nicht unter `/` (#1949) |
| `OPAA_UPLOAD_STORE_PORT` | — (kein Spring-Property; nur `docker-compose.yml`, dort Compose-Default `8095`) | wirkt nur aus Prozessumgebung/`.env`, **nicht** aus `.env.docker` (siehe Hinweis oben) — nicht in `.env.docker.example` gesetzt; ohne Shell-Export gilt der Compose-Default `8095` | Host-Port der S3-API des mitgelieferten Objektspeichers der Originalablage `upload-store` (nur `upload-s3`-Compose-Profil) — für `aws s3` vom Host, etwa bei der Umstellung eines Bestands; das Backend erreicht den Dienst über das Compose-Netzwerk unter seinem Servicenamen, nicht über diesen Port. An `127.0.0.1` gebunden, kein öffentlicher Zugang |
| `OPAA_UPLOAD_STORE_CONSOLE_PORT` | — (kein Spring-Property; nur `docker-compose.yml`, dort Compose-Default `8096`) | wirkt nur aus Prozessumgebung/`.env`, **nicht** aus `.env.docker` (siehe Hinweis oben) — nicht in `.env.docker.example` gesetzt; ohne Shell-Export gilt der Compose-Default `8096` | Host-Port der Konsole des mitgelieferten Objektspeichers der Originalablage (nur `upload-s3`-Compose-Profil) — zum Nachsehen eines Originals im Bucket unter dem Pfad `/rustfs/console/`, Anmeldung mit `OPAA_UPLOAD_STORE_ROOT_USER`/`OPAA_UPLOAD_STORE_ROOT_PASSWORD`. An `127.0.0.1` gebunden, kein öffentlicher Zugang |
...
> **Der MCP-Pfad `/mcp` gehört mit durchgereicht.** Der nginx im Frontend-Container reicht neben `/api/` auch den MCP-Endpunkt des Fremdzugangs an das Backend weiter (`frontend/nginx.conf`, eigener `location`-Block). Ein eigener Reverse-Proxy vor dem Frontend-Container muss `/mcp` deshalb genauso an diesen Container weiterreichen wie `/` — sonst beantwortet die Auslieferung der Anwendung den Aufruf mit ihrer eigenen Startseite, und ein MCP-Client meldet eine unverständliche Antwort statt eines Zugangsfehlers. Antworten dieses Pfads darf der äußere Proxy nicht puffern: Der Kanal kann als Ereignisstrom antworten.
...
Auf `/api/`-Antworten setzt zusätzlich Spring Security eigene `X-Content-Type-Options`/`X-Frame-Options`-Header; `proxy_hide_header` in den beiden Weiterleitungsblöcken (`/api/` und der MCP-Pfad) entfernt diese, damit der Client nur die eine, am nginx gesetzte Kopie sieht statt beide Werte doppelt.
...
**Eine Änderung an der Bibliothek lässt den Geheimtext unangetastet:** Solange der Schlüssel fehlt,
liest die Anwendung die Zugangsdaten und das Push-Geheimnis (`source_credentials`,
...
| Extern | Ob der Anbieter einem anderen Haus gehört. Vorgabe: jeder Anbieter außer dem Standardanbieter ist extern, bis die Systemverwaltung es ändert; die Zeile „Lokale Konten" ist es nie. Nur durch die Systemverwaltung änderbar (`POST /api/v1/admin/oidc-providers/{id}/external`), als Änderung der Anbieterzeile protokolliert. Das Kennzeichen entscheidet, wie die Gruppen dieses Anbieters in Auswahlfeldern dargestellt werden |
...
**Standardanbieter.** Der erste Anbieter ist automatisch der Standard; er kann weder deaktiviert noch gelöscht werden, solange ein weiterer Anbieter existiert, der stattdessen Standard werden kann. Der Verzeichnisabgleich hängt **nicht** mehr an ihm, sondern ist je Anbieter einschaltbar (siehe [Verzeichnisabgleich je Anbieter](#verzeichnisabgleich-je-anbieter)). Ein Zustand ohne Anbieter ist zulässig: Ein aktivierter Anbieter darf nur deaktiviert oder gelöscht werden, wenn danach noch ein anmeldefähiger Systemverwalter übrig bleibt (sonst Antwort 409 mit dem Hinweis, zuerst ein lokales Systemverwalterkonto mit Passwort einzurichten); der **letzte aktivierte** Anbieter zusätzlich nur mit ausdrücklicher Bestätigung (`acknowledgeLastProvider=true`; danach können sich nur noch lokale Konten anmelden). Der Erstadministrator ist seit ADR-0033 das lokale Notanker-Konto (`OPAA_INITIAL_ADMIN_EMAIL`, siehe Variablentabelle); Konten eines Anbieters erhalten `SYSTEM_ADMIN` nur durch Rollenvergabe. **Seine Gruppen gehen mit dem Anbieter**, solange keine von ihnen noch verwendet wird: Hat eine seiner Gruppen noch Rechte an einer Bibliothek, ist sie Eigentümerin einer Bibliothek, Mitglied in einem Space oder Trägerin eines Anlegerechts, wird das Löschen mit 409 (`PROVIDER_GROUPS_IN_EFFECT`) abgelehnt, und die Meldung nennt die Zahlen („3 Gruppen werden noch verwendet: Rechte an 7 Bibliotheken, Mitglied in 2 Spaces …“). Die Ablehnung führt in die **Arbeitsliste der Gruppen** dieses Anbieters (Zeilenmenü der Anbieterliste): Sie nennt je Gruppe, wofür sie noch verwendet wird (etwa „Rechte an 7 Bibliotheken · Mitglied in 2 Spaces“), und bietet an, ihre Rechte an eine andere Gruppe zu übertragen; alternativ werden die Rechte dort entfernt, wo sie vergeben sind — an der Bibliothek oder im Space. Nicht verwendete Gruppen gehen mit dem Anbieter. **Deaktivieren bleibt jederzeit möglich** und lässt Gruppen, Mitgliedschaften und Berechtigungen unverändert stehen. Die Gruppen eines deaktivierten Anbieters sind allerdings kein neues Ziel einer Berechtigung mehr — bestehende bleiben unangetastet. Die Zeile „Lokale Konten" steht in derselben Tabelle, ist aber keine Anbieterzeile im engeren Sinn und **erscheint in der Anbieterliste nicht**: nicht löschbar, nie Standard, nur der Anzeigename ist änderbar, und ihr Aktivieren/Deaktivieren ist der Schalter der lokalen Benutzerverwaltung — bedient wird er unter **Administration → Benutzer → Einstellungen** in der Karte „Lokale Anmeldung" (das Abschalten beendet die Sitzungen aller regulären lokalen Konten; lokale Systemverwalter bleiben angemeldet).

**Anmeldung ohne Klick bei laufender Anbieter-Sitzung.** Wer die Anmeldeseite betritt und beim vorgeschlagenen Anbieter — zuletzt verwendet, sonst Standard, sonst der erste — noch eine laufende Sitzung hat, erreicht die Anwendung ohne Klick: OPAA startet einmal je Browser-Tab einen Anmeldeversuch, der dem Anbieter untersagt, dafür eine eigene Seite zu zeigen (`prompt=none`). Lehnt der Anbieter ab — keine Sitzung, Einwilligung oder Kontoauswahl nötig —, steht die Anmeldeseite unverändert da, mit allen Anbietern und, falls eingeschaltet, der Maske der lokalen Konten; eine Fehlermeldung erscheint nicht, weil niemand den Versuch ausgelöst hat. Der Versuch unterbleibt, wenn kein Anbieter aktiviert ist, wenn bereits eine Sitzung besteht, wenn dieser Browser-Tab seinen Versuch schon unternommen hat, nach einem Abmelden, nach einem Sitzungsende mit Grund (damit der Hinweis lesbar bleibt) und nach einer bewusst gestarteten Anmeldung per Klick auf einen Anbieter. Am Anbieter ist dafür nichts einzurichten, und ein Schalter, der das Verhalten abstellt, ist nicht vorgesehen. Wer mit einem **anderen Konto** desselben Anbieters hereinkommen will, meldet sich in OPAA ab: Das beendet die Sitzung beim Anbieter mit, und der nächste Klick auf den Anbieter fragt wieder nach Anmeldedaten. Wurde die Anmeldeseite von einem Direktlink aus erreicht, führt sie nach dem Versuch weiterhin auf das verlinkte Ziel — ob der Versuch gelingt oder abgelehnt wird.

**Wenn der Versuch aus einem anderen Grund abgelehnt wird.** Als Absage gilt jede Antwort des Anbieters mit einem OAuth-Fehlercode, nicht nur „keine Sitzung": Auch eine falsch eingetragene Client-ID oder eine beim Anbieter fehlende Weiterleitungs-URI führt dazu, dass die Anmeldeseite wortlos stehen bleibt — für die Person ist es derselbe Vorgang, den sie nicht ausgelöst hat, und eine rohe OAuth-Meldung hülfe ihr nicht. Zur Eingrenzung schreibt OPAA den Fehlercode in die **Browser-Konsole** (`Silent sign-in refused by the provider`). Wer den Verdacht hat, dass eine Anmeldung gar nicht erst zustande kommt, prüft dort zuerst. Bei einer Anmeldung per Klick auf einen Anbieter wird derselbe Fehler sichtbar: Der Anmeldeversuch endet dann auf der Rückmeldeseite (`/auth/callback`), die ihn im Klartext anzeigt und eine Schaltfläche „Zur Anmeldung" anbietet.

Bei eingeschalteter lokaler Benutzerverwaltung heißt das zugleich: Gelingt der Versuch, ist die Anmeldeseite samt der Maske der lokalen Konten fort, bevor jemand sie benutzen kann. Wer sich trotz laufender Anbieter-Sitzung mit einem Konto dieser Installation anmelden will, ruft **`/login/system`** auf — diese Seite schaltet den Versuch für den Browser-Tab ab und führt dann zur regulären Anmeldeseite, die damit stehen bleibt. Ein Abmelden ist dafür nicht nötig; es würde die Anbieter-Sitzung mit beenden.
...
#### Verzeichnisabgleich je Anbieter

**Ein Anbieter hat genau einen Gruppenmechanismus**: entweder den **Gruppen-Claim** seiner Tokens (Vorgabe, funktioniert ohne Konfiguration) oder den **Verzeichnisabgleich** (empfohlen für Häuser mit gepflegtem Verzeichnis). Beides zugleich lehnt OPAA mit 409 (`DIRECTORY_SYNC_MECHANISM_CONFLICT`) ab — sonst entstünde dieselbe Verzeichnisgruppe zweimal, mit zwei Wahrheiten über die Mitgliedschaft und zwei Entzugszeitpunkten. Das Einschalten des Abgleichs verlangt also einen leeren Gruppen-Claim, und das Setzen eines Gruppen-Claims verlangt einen ausgeschalteten Abgleich.

Bedient wird er unter **Administration → Verzeichnisabgleich**: je Anbieter eine Karte mit Schalter, Intervall, Statuszeile, Trockenlauf, Lauf, dem ausstehenden Plan samt Alter und dem hinterlegten Verzeichniszugang (Speichern, Verbindungstest, Entfernen). Dieselben Einstellungen über die Schnittstelle je Anbieterzeile (`PUT /api/v1/admin/oidc-providers/{id}/directory-sync`, nur `SYSTEM_ADMIN`):

| Einstellung | Bedeutung |
|---|---|
| Abgleich an/aus | Vorgabe **aus**. Ein Update schaltet nirgends einen Abgleich ein, der vorher nicht lief |
| Intervall | Wie oft ein Lauf fällig ist, in Minuten. Vorgabe **360** (6 Stunden), Grenzen **5** bis **10080** (eine Woche) |

**Woran der Abgleich hängt.** Er läuft für die Konten des **jeweiligen** Anbieters: Mitglieder werden nur unter dessen Issuer aufgelöst, und die entstehenden Organisationseinheiten tragen ihn als Herkunft. Gruppen eines anderen Anbieters sind für einen Lauf unsichtbar — er kann sie weder auflösen noch umbenennen. **Ist die Anbieterzeile deaktiviert, pausiert ihr Abgleich**; ein deaktivierter Anbieter ist kein Vertrauensanker, und ein Lauf von Hand wird mit 409 (`DIRECTORY_SYNC_PROVIDER_DISABLED`) abgewiesen.

**Zeitplan.** Ein Lauf startet ohne Zutun, sobald der letzte Lauf des Anbieters mindestens ein Intervall zurückliegt; ein Anbieter, der noch nie gelaufen ist, ist sofort fällig. Ein verpasster Zeitpunkt wird nicht nachgeholt — der nächste Takt findet den Anbieter wieder fällig. Der Takt selbst lässt sich mit `OPAA_DIRECTORY_SYNC_SCHEDULE_ENABLED=false` ganz abschalten; das Intervall bleibt dann eine Einstellung ohne Wirkung, und Läufe gehen nur noch von Hand.

**Die vier Schutzmechaniken.** Der Trockenlauf (`…/directory-sync/dry-run`) rechnet den Unterschied aus, ohne etwas zu schreiben. Ein **leeres Abgleichergebnis** bei vorhandenen Verzeichnisgruppen bricht hart ab (`ABORTED_EMPTY_RESULT`) — „die Quelle hat nicht geantwortet, wie sie soll" darf niemand wegklicken. Ein **unerreichbares Verzeichnis** lässt den letzten bekannten Stand in Kraft (`UNREACHABLE`). Und ein Lauf, der mehr als die **Plausibilitätsschwelle** (`OPAA_DIRECTORY_SYNC_CHANGE_THRESHOLD_FRACTION`, Vorgabewert in der [Variablentabelle](#alle-umgebungsvariablen)) an Mitgliedschaften entziehen oder Gruppen auflösen würde, schreibt nichts und legt seinen Plan zur Bestätigung vor.

**Der Bestätigungsweg.** Ein solcher Lauf endet mit dem Ergebnis `PENDING_CONFIRMATION` und hinterlässt einen **ausstehenden Plan**:

- Der Plan steht mit seinem **Alter** in der Statuszeile des Anbieters (`GET /api/v1/admin/directory-sync/status`), nicht erst auf einer Unterseite. Angesehen wird er unter `…/directory-sync/pending-plan`.
- **Ein neuer Lauf ersetzt ihn.** Es staut sich also nie ein Vorrat an Plänen, von dem irgendwann der älteste bestätigt würde.
- **Bestätigt wird gegen einen frischen Schnappschuss** (`…/pending-plan/{planId}/confirm`, mit Pflichtangabe eines Anlasses): OPAA liest das Verzeichnis erneut und wendet nur an, was dem vorgelegten Plan noch entspricht. Weicht das Ergebnis ab, wird nichts angewendet, der neu gerechnete Plan tritt an die Stelle des alten, und die Antwort ist 409 (`DIRECTORY_SYNC_PLAN_CHANGED`). Ein Plan, den ein späterer Lauf bereits ersetzt hat, antwortet 404.
...
- **Der Abruf eines Originals unterscheidet zwei Fälle.** Ist der Speicher nicht erreichbar,
  ...
  unbekannten Dokument, damit der Abruf keine Auskunft über fremde Bestände gibt. **Der Upload
  antwortet auf dieselbe Störung gleich** („Dienst nicht verfügbar", später erneut versuchen), statt
  einen Anwendungsfehler zu melden; von dem gescheiterten Versuch bleibt weder eine Datei noch eine
  neue Dokumentzeile zurück. Woran es lag — auch ein Zugangsschlüssel ohne Schreibrecht — steht im
  Anwendungsprotokoll; die Gesundheitsgruppe sieht nur die Leserichtung.
...
Objektspeichers ansehen (Host-Port siehe [Services](#services), Pfad `/rustfs/console/`) oder mit dem
Kommandozeilenwerkzeug des Speichers herausholen, etwa `aws s3 cp`. Der Weg über die Oberfläche
...
| Uploads scheitern mit „Dienst nicht verfügbar", der Speicher ist erreichbar | Der Bucket fehlt oder der Zugangsschlüssel darf nicht schreiben — die Anwendung meldet beides wie eine Störung des Speichers. Den Grund nennt das **Anwendungsprotokoll**, mit Bucket und fehlender Berechtigung (etwa `s3:PutObject`). Ein fehlender Bucket fällt auch der Gesundheitsgruppe auf (`DOWN`); ein reiner Schreibrechte-Fehler nicht — ihre Sondierung prüft nur die Leserichtung und bleibt dann auf `UP` |
