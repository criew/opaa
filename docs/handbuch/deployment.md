...
| `OPAA_INDEXING_CONFLUENCE_WEBHOOK_DEBOUNCE` | `5s` | `5s` | Wie lange der Webhook-Eingang Benachrichtigungen zu einer Bibliothek sammelt, bevor er den gezielten Lauf startet — fünf Speichervorgänge in einer Minute kosten einen Lauf, nicht fünf |
...
| `OPAA_INDEXING_S3_EVENTS_DEBOUNCE` | `5s` | `5s` | Sammelzeit des Ereigniswegs: gemeldete Schlüssel einer Bibliothek werden so lange gesammelt und dann in **einem** Ereignislauf geprüft — ein Skript, das fünfzig Objekte hochlädt, kostet einen Lauf, nicht fünfzig |
...
Dasselbe Prinzip gilt für die Vielfaltsauswahl MMR, die aus demselben Grund per Voreinstellung
abgeschaltet ist.
