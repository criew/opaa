...
| — (Systemeinstellung) | `600` Abrufe/Stunde | — | Die **Schwelle des Abflussalarms**, ebenfalls unter *Systemkonfiguration → Fremdzugänge* (1–1.000.000). Gezählt werden die Abrufe des **ganzen Kanals** in einem gleitenden Stundenfenster, ausschließlich im Arbeitsspeicher (ADR-0021) — keine Tabelle, keine Zeitreihe, kein Verlauf, und die Zählung überlebt keinen Neustart. Bei Überschreitung entsteht **eine** Meldung an jede Systemverwaltung der Organisation (Zeitpunkt, Schwelle, gemessener Wert, Token-Kennung) plus eine `WARN`-Zeile im Anwendungslog; danach schweigt der Alarm sechs Stunden, damit ein anhaltender Vorgang nicht in eine Meldungsreihe zerfällt. **Kein Eintrag im Nachweisprotokoll** — der Alarm ist ein Sicherheitsereignis; ins Protokoll gelangt allein eine daraus folgende Sperre |
| **Indizierung** | | | |
