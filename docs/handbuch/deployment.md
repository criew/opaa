...
| `OPAA_DOCUMENTS_ATTACHMENT_EXTRACTION_MAX_CONCURRENT` | `4` | nicht gesetzt (Anwendungs-Default gilt) | Wie viele Anhangs-Nachextraktionen gleichzeitig laufen dürfen (#1243). Ein Anhangsdokument hat kein gespeichertes Original; es wird beim Öffnen aus dem Original seines Elterndokuments nachextrahiert — jeder Vorgang belegt kurzzeitig Plattenplatz und, bei Konnektor-Beständen, eine Verbindung zur Quelle. Abrufe desselben Elterndokuments laufen zusätzlich immer nacheinander. Wird der Deckel überschritten, antwortet der Abruf mit 503 und einer deutschen Meldung, statt unbegrenzt zu warten. Gültig 1–64 |
| `OPAA_DOCUMENTS_ATTACHMENT_EXTRACTION_ACQUIRE_TIMEOUT` | `10s` (ISO-8601-Dauer oder Spring-Kurzform) | nicht gesetzt (Anwendungs-Default gilt) | Wie lange ein Abruf auf seinen Platz wartet, bevor er mit 503 abgewiesen wird — lang genug, dass eine übliche Klickfolge sich einreiht, kurz genug, dass kein Anfrage-Thread dauerhaft hinter einer langsamen Extraktion hängt |
| **Datenbank** | | | |
