...
| `OPAA_OIDC_AUTHORITY` | — (leer) | `http://localhost:8180/realms/opaa` | Authority der Anmeldeseite: Bis die Anmeldeseite die Anbieter aus der Datenbank liest (#1332), bestimmt dieser Wert über `GET /api/v1/auth/config` weiterhin, gegen welchen Anbieter der Browser den Anmeldefluss startet — er muss deshalb der Issuer-URI entsprechen. Für die übernommene Anbieterzeile wird eine Abweichung nur protokolliert und verworfen, für die Anmeldeseite wirkt sie |
...
> **Wichtig:** `OPAA_OIDC_JWK_SET_URI` muss den Docker-internen Hostnamen `keycloak` (nicht `localhost`) verwenden, weil der Backend-Container JWT-Tokens verifiziert, indem er Schlüssel von Keycloak abruft. `OPAA_OIDC_ISSUER_URI` und `OPAA_OIDC_AUTHORITY` sollten `http://localhost:8180/...` bleiben, da der Browser diese URLs verwendet. Alle vier `OPAA_OIDC_*`-Werte müssen gesetzt sein — sie haben seit #1329 keine Vorgabewerte mehr (`.env.docker.example` führt sie): Drei sind die Bootstrap-Werte der Anbieterzeile, `OPAA_OIDC_AUTHORITY` ist bis #1332 die Authority der Anmeldeseite und muss der Issuer-URI entsprechen.
