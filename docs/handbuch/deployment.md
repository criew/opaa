...
**Wenn der Versuch aus einem anderen Grund abgelehnt wird.** Als Absage gilt jede Antwort des Anbieters mit einem OAuth-Fehlercode, nicht nur „keine Sitzung": Auch eine falsch eingetragene Client-ID oder eine beim Anbieter fehlende Weiterleitungs-URI führt dazu, dass die Anmeldeseite wortlos stehen bleibt — für die Person ist es derselbe Vorgang, den sie nicht ausgelöst hat, und eine rohe OAuth-Meldung hülfe ihr nicht. Zur Eingrenzung schreibt OPAA den Fehlercode in die **Browser-Konsole** (`Silent sign-in refused by the provider`). Wer den Verdacht hat, dass eine Anmeldung gar nicht erst zustande kommt, prüft dort zuerst; die Anmeldung per Klick auf einen Anbieter zeigt denselben Fehler anschließend als Meldung auf der Seite.
