...
| Uploads scheitern mit „Dienst nicht verfügbar", der Speicher ist erreichbar | Der Bucket fehlt oder der Zugangsschlüssel darf nicht schreiben — die Anwendung meldet beides wie eine Störung des Speichers. Den Grund nennt das **Anwendungsprotokoll**, mit Bucket und fehlender Berechtigung (etwa `s3:PutObject`). Die Gesundheitsgruppe hilft hier nicht: Ihre Sondierung prüft nur die Leserichtung und bleibt bei einem reinen Schreibrechte-Fehler auf `UP` |
