...
| `OPAA_LOCAL_ADMIN_ALLOWED_CIDRS` | — (leer = keine Beschränkung) | nicht gesetzt | Kommagetrennte Adressen oder CIDR-Bereiche (IPv4 und IPv6), aus denen sich lokale `SYSTEM_ADMIN`-Konten anmelden dürfen; die Abweisung ist dieselbe wie bei einem falschen Passwort. Reguläre lokale Konten sind nicht betroffen. Ein ungültiger Eintrag lässt den Start fehlschlagen. Bis zur Trusted-Proxy-Auflösung (#1535) zählt die Adresse der Verbindung selbst, nicht `X-Forwarded-For` |
| **Lokale Konten** | | | |
...
**Standardanbieter.** Der erste Anbieter ist automatisch der Standard („Verzeichnis-Anbieter"); er kann weder deaktiviert noch gelöscht werden, solange ein weiterer Anbieter existiert, der stattdessen Standard werden kann. Nur seine Konten löst der Verzeichnisabgleich auf. Ein Zustand ohne Anbieter ist zulässig: Der **letzte aktivierte** Anbieter darf deaktiviert oder gelöscht werden — nur mit ausdrücklicher Bestätigung (`acknowledgeLastProvider=true`; danach können sich nur noch lokale Konten anmelden) und nur, wenn ein lokales Systemverwalterkonto mit Passwort besteht (sonst Antwort 409 mit dem Hinweis, zuerst eines einzurichten). Der Erstadministrator ist seit ADR-0033 das lokale Notanker-Konto (`OPAA_INITIAL_ADMIN_EMAIL`, siehe Variablentabelle); Konten eines Anbieters erhalten `SYSTEM_ADMIN` nur durch Rollenvergabe. Die Zeile „Lokale Konten" in der Anbieterliste ist keine Anbieterzeile im engeren Sinn: nicht löschbar, nie Standard, nur der Anzeigename ist änderbar, und ihr Aktivieren/Deaktivieren ist der Schalter der lokalen Benutzerverwaltung (das Abschalten beendet die Sitzungen aller regulären lokalen Konten).
