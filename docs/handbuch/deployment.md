...
| `OPAA_MCP_DEFAULT_MAX_HITS` | `5` | `5` | Dokumente, die eine Suche über den MCP-Server ohne eigenes `maxHits` liefert. Niedriger als `OPAA_SEARCH_DEFAULT_MAX_HITS`, weil ein fremdes Modell jedes gelieferte Zeichen aus seinem Kontextfenster bezahlt und den gewählten Abschnitt ohnehin nachlädt |
| `OPAA_MCP_EXCERPT_CHARACTERS` | `500` | `500` | Länge des Auszugs um die Fundstelle je Treffer des MCP-Suchwerkzeugs. Betrifft nur diesen Kanal; `POST /api/v1/search` antwortet unverändert mit `OPAA_SEARCH_EXCERPT_MAX_CHARACTERS` |
| **Fremdzugänge** | | | |
