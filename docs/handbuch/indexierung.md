...
Struktur-Metadaten aus der Pipeline (Ortsangabe, bei Mails die Kopfdaten). Space und
Gliederungspfad kommen dagegen vom Dokument selbst, nicht von der Pipeline: Sie stehen auf jedem
Chunk eines Dokuments mit Quellkontext, auch wenn ein Confluence-Anhang über eine andere Pipeline
läuft als die Confluence-Seite selbst. Diese Metadaten sind das, worüber die Suche später filtert,
etwa auf die Bibliotheken, die eine Person sehen darf.
