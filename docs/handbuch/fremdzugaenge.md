...
| **Suchen über MCP** | Dasselbe, nur knapper dargestellt: **ein Eintrag je Dokument** (weitere Fundstellen desselben Dokuments stehen als Liste von Kennungen daneben) und je Eintrag ein **Auszug um die Fundstelle** statt des ganzen Abschnitts. Voreinstellung: fünf Dokumente. Den vollen Text holt das Werkzeug anschließend mit *Abrufen* — und nur für den Treffer, für den es sich entschieden hat |
