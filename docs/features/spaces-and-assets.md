...
2. **Standard-Suchbereich** für Chats ohne gebundenen Agenten, sofern der Schalter „Wissen nutzen" eingeschaltet ist — verengend, nie erweiternd (siehe [Suchbereich je Chatart](#suchbereich-je-chatart)).
...
| Chat ohne Agent, „Wissen nutzen" **aus** | ausschließlich die per @ referenzierten Bibliotheken des Chats **geschnitten mit** den lesbaren Bibliotheken des Nutzers. **@-Referenzen wirken nur in diesem Zustand** — bei „Wissen nutzen" an bleiben sie ohne Einfluss auf den Suchbereich. **Ohne Referenz findet keine Dokumentensuche statt** — das Modell antwortet ohne Quellen, die Oberfläche weist darauf hin |
| Chat mit Agent A | die vom Agenten gebundenen Bibliotheken **geschnitten mit** den lesbaren Bibliotheken des Nutzers. Schalter und @-Referenzen wirken hier nicht — Folge der bestehenden Agenten-Asymmetrie (siehe unten), keine neue Festlegung |
...
**In einem Space mit Zitierzwang bzw. im Strikt-Modus ist der Schalter „Wissen nutzen" nicht abschaltbar, und @-Referenzen sind auf die dem Space assoziierten Bibliotheken beschränkt.** Das ist fail-closed, analog zur bestehenden Aufrufverweigerung für Agenten im Strikt-Space (siehe [Der Strikt-Modus](#der-strikt-modus)): Ein Nutzer kann in einem solchen Raum die Dokumentensuche weder abschalten noch über eine beliebige @-Referenz auf unkuratiertes Wissen außerhalb der Space-Kuratierung ausweiten.

Bei eingeschaltetem Schalter „Wissen nutzen" **verengt** der Space den ungebundenen Chat (im Zielbild über die Assoziation, bis #203 nur mittelbar über die Übergangsregel); bei ausgeschaltetem Schalter bestimmt ausschließlich die @-Referenz den Suchbereich, unabhängig vom Space. Der Space **verengt aber nie den Agenten**. Diese Asymmetrie ist beabsichtigt: Nur wenn die Wissensbindung eines Agenten unabhängig davon ist, wo er ausgeführt wird, bleibt ein Agenten-Release versionierbar und prüfbar. Würde der Space zusätzlich verengen, antwortete dieselbe geprüfte Fassung eines Agenten je nach Space anders — und ein Prüfbericht würde wertlos.

**Übergangsregel bis #203:** Es gibt noch keine Space↔Bibliothek-Assoziation. Bei „Wissen nutzen" an ist der Suchbereich eines ungebundenen Chats deshalb für jeden Space gleich — **alles, was der Nutzer lesen darf**.

**Dauerhafte Regel, unabhängig von #203:** Ein Space **ohne** assoziierte Bibliotheken verengt nicht — dort bleibt der Suchbereich bei „Wissen nutzen" an **alles, was der Nutzer lesen darf**, auch nachdem #203 die Assoziation technisch ermöglicht hat. Erst eine tatsächlich gesetzte Assoziation verengt den Standardbereich; ein Space, der nichts kuratiert, hat nichts zu verengen. Das ersetzt die frühere Sonderregel für den persönlichen Space und kommt ohne Space-Art aus. Damit steht ein Raum, in dem jemand allein arbeitet, fachlich nie schlechter da als ein gemeinsamer, und die Möglichkeit, unbeobachtet zu arbeiten, ist keine bloß formale: Wer dorthin ausweicht, verliert keinen Zugang zu Wissen.

**Mit #203, sobald tatsächlich Assoziationen gesetzt sind:** Ein Space, in dem der Nutzer auf keine der assoziierten Bibliotheken Zugriff hat, ist ein **zulässiger Zustand, kein Fehler**: Bei „Wissen nutzen" an ist der Suchbereich dann leer, und im Zitierzwang-Modus verweigert das System folgerichtig die Antwort. Die Meldung darf dabei **keine Anzahlen nennen**. Zulässig: „In diesem Space ist für dich derzeit kein Wissen verfügbar." Unzulässig: „3 von 4 Bibliotheken sind für dich gesperrt."

**Ein leerer Suchbereich hat zwei unterschiedliche Ursachen und wird entsprechend unterschiedlich behandelt.** Bei „Wissen nutzen" aus ohne @-Referenz ist das eine **bewusste Nutzerentscheidung**: Die Antwort erscheint ohne Quellen, deutlich gekennzeichnet — kein Zitierzwang-Fehler, weil niemand eine Suche erwartet hat, die gar nicht angefordert wurde. Bei „Wissen nutzen" an mit leerem Suchbereich ist das dagegen ein **Systemzustand**, den der Nutzer nicht gewählt hat: Im Zitierzwang-Modus verweigert das System hier folgerichtig die Antwort, wie oben beschrieben. In einem Zitierzwang- bzw. Strikt-Space tritt der erste Fall ohnehin nicht auf, weil dort „Wissen nutzen" nicht abschaltbar ist.
...
- **Der Schalter „Wissen nutzen" ist fest auf an gesetzt und nicht abschaltbar; @-Referenzen sind auf die dem Space assoziierten Bibliotheken beschränkt.** Fail-closed aus demselben Grund wie die Aufrufverweigerung für Agenten: Ein Strikt-Space garantiert einen kuratierten Leserkreis, und diese Garantie darf weder durch Abschalten der Suche noch durch eine beliebige @-Referenz auf unkuratiertes Wissen unterlaufen werden.
