...
**Sobald tatsächlich Assoziationen gesetzt sind:** Ein Space, in dem der Nutzer auf keine der assoziierten Bibliotheken Zugriff hat, ist ein **zulässiger Zustand, kein Fehler**: Bei @Alles-Wissen in der Leiste ist der Suchbereich dann leer, und im Zitierzwang-Modus verweigert das System folgerichtig die Antwort. Die Meldung darf dabei **keine Anzahlen nennen**. Zulässig: „In diesem Space ist für dich derzeit kein Wissen verfügbar." Unzulässig: „3 von 4 Bibliotheken sind für dich gesperrt." (`QueryMetadata.noKnowledgeAvailableInSpace` trägt dieses Signal serverseitig, getrennt von `answeredWithoutKnowledge`, das die bewusst geleerte Chip-Leiste meint.)
