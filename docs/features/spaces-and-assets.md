...
3. **Grant an „Alle Konten"** — die Pseudo-Gruppe aller Konten der Organisation
   ([ADR-0037](../decisions/0037-reichweite-als-freigabe-an-alle.md)). Sie ist kein Objekt in der
   Gruppenverwaltung, sondern eine dritte Subjektart der Freigabe (`subject_type = ALL_ACCOUNTS`),
   und wird in derselben Liste, derselben Historie und über denselben Empfänger-Picker geführt wie
   die beiden anderen Wege.
...
|---|---|
| persönlich | keine Grants außer dem des Eigentümers |
| Team | Grant an die Team-Gruppe |
| **Fachbereich** | **Grant an die Abteilungs- oder Amts-Gruppe** |
| organisationsweit | Grant an „Alle Konten" |
...
- **Übergeben darf, wer `OWNER` ist**, und nur innerhalb der eigenen Organisation. Die Verwaltungsrolle (`MANAGER`) genügt nicht. Die **Systemverwaltung** darf es ebenfalls: Sie zählt an jedem Asset als `OWNER` (docs/features/spaces-and-assets.md#rechte-an-einem-asset-erhalten) — das ist genau der Weg, auf dem ein Asset mit offener Nachfolge wieder eine handlungsfähige Stelle bekommt, ohne dass jemand das Konto der ausgeschiedenen Person bräuchte. Der Vorgang landet wie jede Eigentumsänderung im Nachweisprotokoll (`ASSET_OWNER_CHANGED`) mit ihrem Namen.
