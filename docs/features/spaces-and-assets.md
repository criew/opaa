...
Weg** zur wirksamen Rolle (`GET /api/v1/assets/{assetType}/{assetId}/access-derivation`,
...
- **Ein Platzhalter ist genau `{{name}}`** — ohne Leerzeichen, der Name beginnt mit einem Buchstaben und enthält nur Buchstaben, Ziffern und Unterstriche. Jede andere Folge `{{…}}` wird abgelehnt statt als Text stehen gelassen; eine einzelne geschweifte Klammer oder ein nicht geschlossenes `{{` bleibt Text.
...
- **Systemvariablen** `{{CURRENT_DATE}}` und `{{USER_NAME}}` werden beim Einsetzen aufgelöst — das Datum des Tages, der Name der einsetzenden Person. Sie werden nicht definiert; eine Definition dieses Namens, gleich in welcher Schreibweise, wird abgelehnt.
- **Text und Definitionen müssen in beide Richtungen übereinstimmen.** Der Server lehnt einen Platzhalter ohne Definition ebenso ab wie eine Definition, die der Text nicht verwendet. Ein Prompt wird nur gespeichert, wie diese Prüfung ihn angenommen hat.
...
**Zu `IDENTITY_PROVIDER`:** Die Gruppe benennt ihren Anbieter als Fremdschlüssel (`groups.provider_id`, seit [#1812](https://github.com/criew/opaa/issues/1812)); `external_id` trägt den blanken Namen aus dem Claim — gleichnamige Gruppen zweier Anbieter sind zwei Gruppen, und kein Anbieter erreicht die Gruppen eines anderen. Die Mitgliedschaft folgt ausschließlich dem Token und wird bei jeder Anmeldung neu gesetzt; die Gruppe selbst bleibt bestehen und ist in der Gruppenverwaltung nicht änderbar. Löschbar ist sie nur als Folge des Löschens ihres Anbieters, und auch das nur, solange sie keine Wirkung mehr trägt (409 sonst, [ADR-0036](../decisions/0036-berechtigungsmodell-gruppen-und-faehigkeiten.md), Entscheidung 2). Sie ist weder Gegenstand des Verzeichnisabgleichs noch als Geltungsbereich der Befugnis „Sicht als" wählbar (Einzelheiten in [Identität, Rechte & Mandanten](./access-control.md)).
...
**Wirkung der Obergrenze (gebaut, #797, Maintainer-Festlegung vom 21.09.2026):** Sie begrenzt ausschließlich `visibility` und `listed` der Konnektorbibliothek — keine Gruppengrößen-Schwelle für Grants (die Streichung der Größenschwelle gilt unverändert, siehe [Freigabe an eine Gruppe braucht keine Zustimmung](#freigabe-an-eine-gruppe-braucht-keine-zustimmung)). Die Systemverwaltung setzt die Obergrenze je Bibliothek (`PUT /api/v1/libraries/{libraryId}/share-cap`); ein Bibliotheks-Eigentümer oder `MANAGER` kann `visibility`/`listed` nicht darüber anheben — der Versuch scheitert mit `409`, weil die eigene Berechtigung nicht in Frage steht, sondern die Anfrage mit der gesetzten Obergrenze kollidiert. Wird eine bestehende Obergrenze nachträglich gesenkt, nimmt das System eine bereits weitergehende `visibility`/`listed`-Einstellung **sofort** zurück (auf die neue Obergrenze geklemmt) — mit Audit-Ereignis und einer Zeile in der Sichtbarkeits-Historie, genau wie bei einer Änderung durch den Eigentümer selbst. Erteilte Rechte an Personen und Gruppen (`asset_grants`) sowie eine bestehende Fremdzugangsfreigabe bleiben davon **unberührt** — die Obergrenze deckelt, wie oben festgehalten, ausschließlich `visibility`/`listed`; wer auch diese zurücknehmen will, tut es gesondert. Es gibt bewusst keinen Zustand „verletzt, aber geduldet": Anders als beim [Strikt-Space](#der-strikt-modus), dessen „Voraussetzung verletzt" einen bestehenden Zustand vorübergehend einfriert, weil ein Space nicht unter der Kontrolle des Bibliotheks-Eigentümers steht, liegt die Bibliothek selbst in der Hand der Systemverwaltung, die die Obergrenze setzt — ein sofortiges Zurücknehmen erzeugt hier keine widersprüchliche Zuständigkeit. Eine gesenkte Obergrenze zählt zu den Ursachen, die einen Strikt-Space in den Zustand „Voraussetzung verletzt" versetzen können, sobald jener Mechanismus gebaut ist (#204, noch nicht umgesetzt). `UPLOAD`-Bibliotheken tragen keine Obergrenze: Dieselbe Person kuratiert dort ohnehin jedes Dokument einzeln, es gibt nichts, wovor die Obergrenze schützen müsste.
...
- Der Standard-Space lässt sich ebenso wenig archivieren wie löschen — er ist nicht stillzulegen, sondern das dauerhafte Zuhause seines Eigentümers.
- Löscht der Eigentümer stattdessen einen Space, der noch Chats enthält, verweist die Fehlermeldung auf das Archivieren als Ausweg.

Archivieren ist damit kein Ersatz für Löschen, sondern die einzige Handlung, die auf einen Space wirkt, den die Löschsperre dauerhaft festhält: Sie stellt den Betrieb still, ohne den Nachweis zu zerstören, den die Löschsperre gerade schützen soll.
...
012, seit #904/#1492 Teil der Liquibase-Baseline, siehe `backend/src/main/resources/db/changelog/changes/001-baseline.yaml`) — Eigentümerschaft
...
**„Im Suchbereich" meint hier den tatsächlichen Suchbereich der jeweiligen Anfrage** — bestimmt durch die Chip-Leiste des Chats bzw. durch den gebundenen Agenten (siehe [Suchbereich je Chatart](#suchbereich-je-chatart)), nicht die Gesamtheit der dem Space assoziierten Bibliotheken. Wer eine Bibliothek per Chip-Leiste bewusst aus dem Suchbereich lässt, nutzt ihre Daten nicht — ihre Modell-Obergrenze muss dann auch nicht greifen. Das steht nicht im Widerspruch zur Zusage aus [llm-integration.md](./llm-integration.md#wenn-die-schnittmenge-leer-ist), dass eine strengere Bibliothek nicht **stillschweigend** aus dem Suchbereich genommen wird, um einen Vorgang doch noch zu ermöglichen: Jene Zusage betrifft automatisches Systemverhalten hinter dem Rücken des Nutzers. Die bewusste, sichtbare Verengung durch den Nutzer selbst ist davon unberührt — sie ist keine Umgehung, sondern die reguläre Steuerung des Suchbereichs.
...
Vier strukturelle Grenzen dieses Prüflaufs waren bewusst hingenommen, nicht nachgerüstet; die dritte ist seit der Asset-Schale behoben. **Erstens** führen die Historientabellen (`asset_grant_history`, `group_membership_history`, `asset_visibility_history`) `asset_id`/`subject_group_id`/`group_id` absichtlich **ohne** Fremdschlüssel, damit das Löschen eines Assets oder einer Gruppe ihre Rechtehistorie nicht mitreißt — ein fremdschlüsselbasierter Prüflauf sieht eine Spalte, die nie ein Fremdschlüssel war, strukturell nicht. **Zweitens** trägt `vector_store` (die Spring-AI-Vektortabelle) die Organisation nur als JSON-Metadatum, nicht als relationale Spalte, und fällt damit ebenfalls außerhalb dessen, was der Prüflauf einsehen kann.

**Drittens** galt von [#1811](https://github.com/criew/opaa/issues/1811) bis [#1899](https://github.com/criew/opaa/issues/1899) dasselbe für die **lebende** Freigabe: `asset_grants.asset_id` benannte ein Asset beliebiger Art und konnte auf keine einzelne Tabelle zeigen; ein Trigger an `knowledge_libraries` (`038-asset-grants-type-independent.yaml`) nahm die Freigaben mit der Bibliothek, und für jede weitere Asset-Art war ein eigener Kaskaden-Trigger vorgesehen. **Diese Zusage ist zurückgenommen.** Seit der Asset-Schale verweist `asset_grants` wieder mit einem zusammengesetzten Fremdschlüssel `(asset_id, asset_type, organization_id)` auf `assets` (`081-asset-grants-asset-foreign-key.yaml`, `ON DELETE CASCADE`), und der Trigger ist entfernt. Die Kante liegt damit wieder in der Abdeckung des Prüflaufs, und der Typ ist Teil des Schlüssels: Eine Freigabe mit falschem Typ ist nicht anlegbar. Ein Trigger je Typ hätte die Kaskade in jeder neuen Typtabelle erneut richtig verlangen müssen und wäre dort, wo er fehlt, im Betrieb unauffällig geblieben — genau die Fehlerklasse, die diese Schicht ausschließen soll. Dasselbe gilt für `space_asset_associations` (`083-space-asset-associations-asset.yaml`). Die **Empfängerseite** bleibt unverändert: `fk_asset_grants_subject_user_organization` und `fk_asset_grants_subject_group_organization` machen eine Freigabe an eine Person oder Gruppe einer anderen Organisation nicht anlegbar. Die Historientabellen der ersten Grenze führen `asset_id` weiterhin ohne Fremdschlüssel — sie sollen das Löschen des Assets überdauern.

**Viertens** ist `groups.provider_id` (seit [#1812](https://github.com/criew/opaa/issues/1812)) ein **einspaltiger** Fremdschlüssel auf `oidc_providers` — nicht, weil die Organisation vergessen wurde, sondern weil `oidc_providers` gar keine `organization_id` trägt: Alle Anbieter provisionieren nach [ADR-0025](../decisions/0025-mehrere-oidc-anbieter.md) in dieselbe Organisation. Der Prüflauf sieht damit eine Kante, die seine Regel nicht erfüllen kann. [ADR-0036](../decisions/0036-berechtigungsmodell-gruppen-und-faehigkeiten.md), Entscheidung 13 ordnet die Stelle ausdrücklich [#1442](https://github.com/criew/opaa/issues/1442) zu: Sobald ein Anbieter einer Organisation gehört, kommt die Prüfregel „Gruppe und Anbieter in derselben Organisation" als zusammengesetzter Fremdschlüssel hinzu — genau die Form, die `fk_group_memberships_user_organization` heute schon führt.
