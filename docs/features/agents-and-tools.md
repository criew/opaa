...
- **Ob ein Chat mit neuer Aktivität in der Liste nach oben rückt.** Sortierung nach letzter Aktivität ist das vertraute Muster; in einem geteilten Space bewegen dann aber fremde Läufe die eigene Liste. Für private Chats ist die Ordnung der Liste in [chat-list.md](./chat-list.md) entschieden (letzte Aktivität, Zeitgruppen, Anheften, Chat-Archiv); offen bleibt allein die Wirkung fremder Aktivität in geteilten Chats. Der Lesestand der Ungelesen-Markierung wäre eine weitere Zeile derselben Art wie die dort festgelegten persönlichen Ordnungsmerkmale — je Person und Chat, nur für die eigene Anzeige.
...
- **Wie sich das Teilen auf Läufe und Artefakte auswirkt.** Gesetzt ist das Sichtbarkeitsmodell aus [spaces-and-assets.md](./spaces-and-assets.md#space-eigene-inhalte-chats-und-artefakte): Chats und Artefakte entstehen privat und werden erst durch Teilen space-sichtbar; das Zielbild der Oberfläche folgt dem. Ein Prüfbericht zu einem Vorgang ist kein Inhalt, dessen Sichtbarkeit aus einem Versehen folgen darf. Offen ist, was beim Teilen eines Chats mitgeht — die Schrittliste eines Laufs, die daraus entstandenen Artefakte, eine wartende Bestätigung — und wer in einem geteilten Chat einen Agenten aufrufen oder eine Bestätigung erteilen darf.
