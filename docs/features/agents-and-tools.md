...
**Was nicht geprüft wurde, steht in derselben Liste.** Endet ein Lauf an einer Grenze, erscheint der nicht erreichte Punkt als eigener Zustand **„nicht geprüft"** in derselben Aufzählung wie die übrigen Befunde — nicht als Fließtext-Fußnote unter einer langen Antwort. Ein überlesener Hinweis ist derselbe Schaden wie ein fehlender.
...
Dazu ein **Platzlimit**: eine Höchstzahl gleichzeitiger Werkzeugläufe je Installation. Wird sie erreicht, wird die Anfrage **abgewiesen** — mit einer verständlichen Meldung, nicht mit einer Warteschlange. Eine Warteschlange verschiebt das Problem nur in die Wartezeit und hält dabei Verbindungen offen.

**In allen vier Fällen endet der Lauf gleich:** mit dem Vorliegenden und dem Hinweis auf die unvollständige Prüfung, nicht mit einem Fehler — und der nicht erreichte Punkt erscheint als eigener Zustand in der Ergebnisliste, nicht als Fußnote.
...
Derselbe Mechanismus trägt später den **sofortigen Rückzug eines freigegebenen Skills**: Eine Freigabe, die sich nicht binnen Sekunden zurücknehmen lässt, ist eine Einbahnstraße.
...
Wer ein Werkzeug benutzt hat, ist etwas anderes als das, was jemand gefragt hat. Aus dieser Unterscheidung folgen **drei** Bestände — nicht zwei. Sie in einem Satz zusammenzuziehen war der häufigste Fehler der Entwürfe davor, und er ist in einer Vorlage an die Personalvertretung angreifbar.
...
| **(b) Das technische Schrittprotokoll** | was die Störungssuche und die Messung brauchen: Lauf-Kennung, Werkzeug, Ergebnisstatus, Dauer, Abbruchgrund | eine **bezifferte, kurze** Frist mit automatischer Löschung, nicht abschaltbar; **kürzer als die des Nachweisprotokolls** | niemand über einen Personeneinstieg; Störungssuche geschieht an einem **Lauf**, nicht an einem Bestand |
...
> **Nicht der bestehende Diagnosepfad.** „Diagnosepfad" ist ein besetzter Begriff: Das Diagnoseprotokoll der Suche hat zwölf Monate Aufbewahrung, eine Einzelsatzansicht und einen anlassbezogenen Lesepfad für die Auswertungsrolle. Das ist das Gegenteil dessen, was (b) zusagt. Ob (b) trotzdem dort mitfährt — mit dessen Partitionen und Aufbewahrungslauf, aber eigener, kürzerer Frist — oder eine eigene Ablage bekommt, ist **offen** und eine Entscheidung mit Gegenargumenten auf beiden Seiten (Betrieb: keine zweite Ablage mit derselben Semantik; Mitbestimmung: kein Lesepfad für Auswertungsrollen auf Suchfragen). Bis sie fällt, wird der Begriff hier nicht unqualifiziert verwendet.
...
- **Wo das technische Schrittprotokoll eines Laufs liegt und wie lange.** Entweder im bestehenden Diagnoseprotokoll der Suche — das Partitionen, Aufbewahrungslauf und Privilegienmodell bereits mitbringt, aber zwölf Monate Frist und einen anlassbezogenen Lesepfad für die Auswertungsrolle trägt — oder in einer eigenen Ablage mit kurzer Frist und ohne jeden Lesepfad. Beide Seiten haben ein starkes Argument; die Entscheidung gehört in den ADR der Umsetzung, siehe [Werkzeug-Audit](#werkzeug-audit-drei-bestände-die-nicht-verwechselt-werden-dürfen).
...
Alle Angaben aggregiert je Organisationseinheit, ohne Personenbezug (siehe [Nutzungstransparenz](./spaces-and-assets.md#nutzungstransparenz)). Drei Bedingungen gelten für jede Zahl dieser Liste:

1. **Die Erhebung ist in der Voreinstellung aus**, und der Schalter hängt an der **Erhebung**, nicht an der Anzeige. Was nicht erhoben wird, kann später nicht ausgewertet werden — eine abgeschaltete Anzeige über einem vorhandenen Bestand ist keine Zusage, sondern eine Einstellung.
2. **Die Mindestgruppengröße bemisst sich an der Zahl der tatsächlich nutzenden Personen**, nicht an der Größe der Organisationseinheit. Ein Skill, den in einem Referat von vierundzwanzig nur die drei Beschäftigten eines Sachgebiets benutzen, wäre sonst eine Auswertung über drei Personen unter der Überschrift einer großen Einheit. Unterhalb der Schwelle wird die Zahl **nicht erhoben**, nicht bloß unterdrückt.
3. **Keine Zahl dieser Liste wird je Person erhoben oder je Person auswertbar** — auch nicht mittelbar über einen Personenfilter auf einem anderen Bestand.
