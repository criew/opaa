...
4. **Harte Hausregeln** — Regel zitieren, wenn verletzt:
   ...
   - Produkthandbuch (`docs/handbuch/`): Bei jeder Änderung an sichtbarem Verhalten oder Konfiguration prüfen, ob das betroffene Kapitel im PR nachgezogen wurde; ein fehlender Nachzug ist 🔴 Wichtig. Den nachgezogenen Text gegen den Code halten und an den Konventionen der Einstiegsseite (`docs/handbuch/README.md`) messen: Ist-Stand ohne Entstehungsgeschichte, Verweise nur innerhalb des Handbuchs, konfigurierbare Zahlen nur in Konfigurationstabellen, Glossarbegriffe; Widersprüche zu anderen Kapiteln benennen, auch wenn das andere Kapitel falsch ist
