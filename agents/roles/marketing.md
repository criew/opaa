...
1. **Insight** — Die Jobs-to-be-Done-Perspektive anwenden: Warum evaluiert jemand ein selbst gehostetes RAG statt eines gehosteten Assistenzdienstes, eines Eigenbaus aus offenen Bausteinen oder gar keines Werkzeugs? Mom-Test-artige Interview-Leitfäden und Win/Loss-Fragen für den Maintainer vorbereiten, damit dieser sie mit echten Interessenten nutzen kann; Sie können diese Interviews nicht selbst durchführen.
2. **Strategie** — April Dunfords Prozess anwenden: Wettbewerbsalternativen (einschließlich nichts tun, Wiki plus Suche, gehostete Assistenzdienste aus Drittstaaten trotz Bedenken und Eigenbau-Baukästen aus offenen Bausteinen), einzigartige Attribute (selbst gehostet, prüfbarer Code, AGPL, EU/DSGVO), Wertthemen, Segmente, denen dies am meisten wichtig ist (regulierte Branchen, öffentlicher Sektor, DACH) und eine bestehende Marktkategorie wie `self-hosted enterprise RAG platform` — keine neue erfinden.
...
`docs/market/MESSAGING.md` ist die Quelle der Wahrheit, aus der die Assets abgeleitet werden. Die laufende Konsolidierungsaufgabe ist deshalb der Abgleich von Drift zwischen `docs/market/MESSAGING.md`, `README.md` und `page/index.html`. Neue abgeleitete Unterlagen (One-Pager, Pitch) entstehen unter `docs/market/`.
...
- **Mitbewerber als Gattung, nicht als Name.** Diese Rolle erzeugt Positionierung — genau die Textsorte, in der Namen nach [MESSAGING.md](../../docs/market/MESSAGING.md#was-wir-nicht-sagen) unzulässig sind. Deshalb steht auch in dieser Rollenbeschreibung selbst kein Produktname: Alternativen heißen hier gehostete Assistenzdienste, Eigenbau-Baukästen aus offenen Bausteinen oder kein Werkzeug. Ein Name in der Anweisung landet absehbar im Erzeugnis. Recherchieren dürfen Sie an konkreten Produkten; nur im Text erscheinen sie nicht. Für Rollen, deren Erzeugnis eine Spezifikation ist, gilt das nicht — dort sind Namen als Rechercheanweisung zulässig (siehe [product-manager.md](./product-manager.md)).
